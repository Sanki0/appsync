...
type CognitoClient interface {
	...
	AdminCreateUser(alias string, email string, name string) (string error)
...
func (d *deps) handler(ctx context.Context, event Event) (string, error) {
	...
	case 1: // AdminCreateUser
		result, err = client.AdminCreateUser(event.Alias, event.Email, event.Name)
...
func (ctx *awsCognitoClient) AdminCreateUser(alias string, email string, name string) (string, error) {
